...
size_t Readframe(void *vptr,unsigned char star_byte,ssize_t read_line_length)
{
    ...
    ssize_t No_Star_length = 0;
    while(ptr[No_Star_length] != '#' && No_Star_length < read_line_length) 
        No_Star_length++;
    for (int i = 0; i < read_line_length-No_Star_length; ++i)
    {
        ptr[i] = ptr[No_Star_length+i];

    }
    return read_line_length-No_Star_length;
